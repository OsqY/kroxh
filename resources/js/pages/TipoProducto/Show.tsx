...
export default function Show({ auth, tipoProducto }) {
    const handleDelete = () => {
    ...
    return (
        <AppLayout
        >
            <Head title={`Tipo de Producto: ${tipoProducto.nombre}`} />
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-2xl">{tipoProducto.nombre}</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p>{new Date(tipoProducto.created_at).toLocaleString()}</p>
                                    </div>
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-500">Última actualización</h3>
                                        <p>{new Date(tipoProducto.updated_at).toLocaleString()}</p>
                                    </div>
                                </div>
                            </CardContent>
                            <CardFooter className="flex justify-between">
                                <Button
                                    variant="outline"
                                    onClick={() => router.visit(route('tipo-productos.index'))}
                                >
