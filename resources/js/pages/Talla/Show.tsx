...
export default function Show({ auth, talla }) {
    const handleDelete = () => {
    ...
    return (
        <AppLayout
        >
            <Head title={`Tipo de Producto: ${talla.nombre}`} />
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-2xl">{talla.nombre}</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p>{new Date(talla.created_at).toLocaleString()}</p>
                                    </div>
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-500">Última actualización</h3>
                                        <p>{new Date(talla.updated_at).toLocaleString()}</p>
                                    </div>
                                </div>
                            </CardContent>
                            <CardFooter className="flex justify-between">
                                <Button
                                    variant="outline"
                                    onClick={() => router.visit(route('tallas.index'))}
                                >
